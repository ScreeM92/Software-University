...
namespace GoblinFreelancer.Controllers
{
    ...
    {
        private string[] ALLOWED_ROLES = { "Hire", "Work" };
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                var crrentUser = (from user in context.Users
                                  where user.UserName == model.UserName
                                  select user).FirstOrDefault();

                if (crrentUser != null && crrentUser.IsDisalbed)
                {
                    this.Response.Redirect("~/Home/UserBanned");
                }
        ...
        {
            if (ModelState.IsValid && ALLOWED_ROLES.Contains(model.Role))
            {
                ...
                if (result.Success)
                {
                    ...
                    var role = await IdentityManager.Roles.FindRoleByNameAsync(model.Role);
                    if (role == null)
                    {
                        await IdentityManager.Roles.CreateRoleAsync(new Role(model.Role));

                        role = await IdentityManager.Roles.FindRoleByNameAsync(model.Role);
        ...
        public async Task<ActionResult> Manage(ManageUserViewModel model)
        {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (result.Success)
                    {
                        if (this.Request.IsAjaxRequest())
                        {
                            ...
                            return PartialView("_ChangePasswordPartialModified", new ManageUserViewModel ());
                        }
            ...
            if(this.Request.IsAjaxRequest())
            {
                return PartialView("_ChangePasswordPartialModified", model);
            }
            // If we got this far, something failed, redisplay form
            return View(model);
        }
